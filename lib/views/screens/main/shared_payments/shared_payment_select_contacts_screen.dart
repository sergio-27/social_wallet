    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: BlocBuilder<SharedPaymentContactsCubit, SharedPaymentContactsState>(
            bloc: widget.sharedPayContactsCubit,
            builder: (context, state) {
              List<SharedContactModel> sharedContactsList = state.selectedContactsList ?? [];
              double pendingAmount = state.totalAmount != null ? state.totalAmount! - (state.allSumAmount ?? 0.0) : 0.0;
              pendingAmount = pendingAmount.toStringAsPrecision(5).parseToDouble();
              return Flex(direction: Axis.vertical, mainAxisAlignment: MainAxisAlignment.start, children: [
                Expanded(
                          ...
                          .map((e) => SharedContactItem(
                              ...
                              onClick: () async {
                                if (e.amountToPay == 0.0) {
                                  setAddedUserAmount(sharedContactModel: e, selectedContactsList: state.selectedContactsList ?? []);
                                }
                              }))
                          .toList()),
                )),
    ...
    if (currUser != null) {
      if (currUser.userEmail != widget.sharedPayment.ownerEmail) {
        widget.sharedPayContactsCubit.updateSelectedContactsList(
            [SharedContactModel(userId: currUser.id ?? 0, contactName: currUser.username ?? "", userAddress: currUser.accountHash ?? "", imagePath: "", amountToPay: 0.0)]);
      }
    }
  }

  void setAddedUserAmount({required SharedContactModel sharedContactModel, required List<SharedContactModel> selectedContactsList}) async {
    List<String>? resultsCurrUserAmount = await AppConstants.showCustomTextInputDialog(
        context: context,
        title: "Your amount",
        message: "Introduce your total amount to pay",
    ...
    if (resultsCurrUserAmount != null && mounted) {
      widget.sharedPayContactsCubit.setUserAmountToPay(
          context,
